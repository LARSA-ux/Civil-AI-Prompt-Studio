...
The application presents four workflow stages:
...
3. **Answer evaluator** — reviews the draft for accuracy, relevance, completeness, clarity, terminology, grounding, unsupported claims, and missing caveats.
...
- **Free Demo Mode** runs locally with transparent deterministic templates. It requires no API key, no billing, and makes no network request.
- **OpenAI API Mode** runs the live multi-stage pipeline using the **OpenAI Responses API**, official OpenAI Python SDK, and Pydantic structured outputs.

The interface is built with Streamlit and configuration is loaded with `python-dotenv`.

---

## 2. Features

- Free Demo Mode with zero API calls, zero credits, and clearly labelled rule-based outputs
- OpenAI API Mode for live model-generated analysis, generation, evaluation, and revision
...
(demo_pipeline.py)         (ai_pipeline.py)
        |                       |
        +-----------+-----------+
                    |
                    v
PromptAnalysis -> draft -> AnswerEvaluation -> optional revision
                    |
                    +--> Strict mode may stop for clarification
                    +--> pass: display draft as final
                    +--> insufficient_context: request evidence/data
                    +--> revise: display revised final answer
```
...
- `ai_pipeline.py` — OpenAI client calls and live four-stage orchestration.
- `demo_pipeline.py` — deterministic no-cost demonstration workflow with reviewed sample-topic templates.
...
- An OpenAI API key with API billing enabled, **only for OpenAI API Mode**

### ChatGPT subscription versus API billing

A ChatGPT Free, Plus, Pro, or other ChatGPT subscription does **not** automatically include OpenAI API credits. ChatGPT subscriptions and OpenAI API billing are separate products.

You can use **Free Demo Mode without any API key or payment**. An API key and available API credit are required only when you select **OpenAI API Mode**.
...
This section is optional when you use Free Demo Mode.

To use OpenAI API Mode, copy `.env.example` to a new file named `.env`.
...
1. Launch the app.
2. Keep the default execution mode: **Free Demo Mode (no API)**.
3. Leave Strict Clarification Mode off.
...
8. Select **Run Free Demo Pipeline**.
...
Because no trusted source was supplied, the deterministic evaluator should request a visible verification limitation and Automatic Revision should add it. The page must clearly state that Demo Mode is rule-based and not a live AI response.
...
into the Trusted Context box and run the same question again. In Demo Mode, the app honestly states that it cannot semantically verify arbitrary pasted documents. In OpenAI API Mode, the answer should use the supplied note as its evidence boundary and should not invent a design-code limit or claim that a real building is safe.
...
Free Demo Mode is the default and requires no API key, API billing, or internet request. It uses deterministic educational templates for the included sample topics and returns the same `PromptAnalysis`, draft, `AnswerEvaluation`, and optional revised-answer structure as the live pipeline.

Demo Mode is deliberately labelled throughout the interface. Its outputs are not model-generated, do not perform literature search, and do not independently verify arbitrary trusted context. Unrecognised topics return an honest insufficient-context result rather than fabricated technical content.

### OpenAI API Mode

OpenAI API Mode uses the configured Responses-API model and can handle a broader range of questions. It requires `OPENAI_API_KEY` and available API credit. One full run may make three or four API calls.
...
- Switch the sidebar to **Free Demo Mode (no API)** to continue without paying or making API calls.
- For OpenAI API Mode, check API billing and available credits.
...
- Free Demo Mode supports reviewed templates for the included sample topics; unrecognised topics intentionally return limited results.
- Demo Mode does not semantically verify arbitrary trusted-context text.
- In OpenAI API Mode, the model is still an AI system and can make technical mistakes.
...
- dual-mode architecture with a no-cost deterministic fallback
...
> Built a dual-mode civil-engineering prompt optimiser with a free deterministic demonstration pipeline and an optional OpenAI Responses API pipeline. The app analyses vague prompts, detects missing engineering information, evaluates unsupported claims and safety gaps, and revises failed responses using Pydantic and Streamlit.
