...
def main() -> None:
    ...
    with st.sidebar:
        ...
        model = st.text_input(
            ...
            help="Loaded from OPENAI_MODEL. Change it here if your API project uses another accessible model.",
        )
    ...
    run_clicked = st.button("Run Civil AI Pipeline", type="primary", use_container_width=True)
    if not run_clicked:
        return
    ...
    try:
        ...
        with st.status("Running the multi-stage AI review pipeline...", expanded=True) as status:
